...
document.addEventListener('DOMContentLoaded', () => {
     ...
     * Jika tidak ada data di localStorage, pakai data default (baseProducts).
     * Data default bisa di-update dengan meng-edit file data.js
     */
    (function loadProducts() {
        ...
        if (saved) {
            try {
                ...
                console.warn('⚠️ Gagal parse localStorage, pakai data default');
                products = [...baseProducts];
            }
        } else {
            products = [...baseProducts];
            console.log('📦 Data default dimuat dari baseProducts');
        }
    })();

    /**
     * Menyimpan data produk ke localStorage.
     * CATATAN: Data ini hanya tersimpan di browser pengguna saat ini.
     * Untuk menyimpan permanen di server, gunakan fitur Export.
     */
    function saveProducts() {
        localStorage.setItem('dapuMuliaProducts', JSON.stringify(products));
        console.log('💾 Data produk disimpan ke localStorage');
    }
    ...
    function addToCart(productId) {
        ...
        setTimeout(() => { dom.cartBadge.style.transform = 'scale(1)'; }, 200);
    }
    ...
    function checkout() {
        ...
        window.open(`https://wa.me/6281273127063?text=${encodeURIComponent(message)}`, '_blank');
        toggleCart(false);
    }
    ...
    dom.cartBtn.addEventListener('click', (e) => { e.preventDefault(); toggleCart(); });
    ...
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && state.isCartOpen) toggleCart(false);
    });

    // ========== ADMIN PANEL (FULL CRUD) ==========
    dom.adminBtn.addEventListener('click', (e) => { e.preventDefault(); toggleAdminPanel(); });
    document.addEventListener('keydown', (e) => { if (e.key === 'a' && e.ctrlKey) toggleAdminPanel(); });
    ...
    function openAdminPanel() {
        ...
        panel.innerHTML = `
            ...
            <div class="admin-panel-footer"><p>💡 Klik <i class="fas fa-edit"></i> edit | <i class="fas fa-trash"></i> hapus | ⚠️ Data hanya disimpan di browser SAAT INI. Klik <strong>"Export data.js"</strong> lalu GANTI file <code>data.js</code> di folder project agar perubahan permanen untuk semua orang!</p></div>
        `;
    ...
    // ========== OPEN EDIT MODAL with UPLOAD FOTO ==========
    function openEditModal(productId) {
                ...
                <div class="admin-modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
                            <input type="file" id="editImageFile" accept="image/*" style="flex:1;padding:8px;border:1px solid rgba(255,107,0,0.15);border-radius:8px;font-size:13px;">
                            <span style="color:var(--text-muted);font-size:12px;">ATAU</span>
                            <input type="url" id="editImageUrl" value="${isEdit && product.image && !product.image.startsWith('data:') ? product.image : ''}" placeholder="https://...link-foto.jpg" style="flex:2;padding:10px 14px;border:1px solid rgba(255,107,0,0.15);border-radius:8px;font-size:14px;">
                        </div>
                        <div style="margin-top:8px;display:flex;align-items:center;gap:12px;">
                            <img id="editImagePreview" src="${isEdit ? product.image : ''}" style="width:60px;height:60px;object-fit:cover;border-radius:8px;${isEdit && product.image ? '' : 'display:none;'}" onerror="this.style.display='none'">
                            <small style="color:var(--text-muted);">📷 Klik "Pilih File" untuk upload dari komputer, atau masukkan URL gambar online</small>
                        </div>
                    </div>
        ...
        document.getElementById('modalSaveBtn').addEventListener('click', () => {
            ...
            const urlVal = document.getElementById('editImageUrl').value.trim();
            const image = uploadedImageData || urlVal || (isEdit ? product.image : '');
            ...
            if (!name || !price || !weight || !image || !description) {
                showToast('Isi semua field! (Nama, Harga, Berat, Foto, Deskripsi)', true);
                return;
            }
            if (isEdit) {
            ...
            if (hdr) hdr.innerHTML = `<i class="fas fa-cog"></i> Kelola Produk (${products.length})`;
        });
    }
    ...
    function injectAdminStyles() {
        ...
        s.textContent = `
            ...
            .admin-btn-danger{background:var(--accent)}.admin-btn-danger:hover{background:#c62828}
            ...
            .edit-btn{background:rgba(255,107,0,0.1);color:var(--primary)}.edit-btn:hover{background:var(--primary);color:#fff}
            .delete-btn{background:rgba(255,0,0,0.1);color:var(--accent)}.delete-btn:hover{background:var(--accent);color:#fff}
